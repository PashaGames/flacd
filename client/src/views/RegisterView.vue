            ...
            <div class="w-full bg-white rounded-lg shadow md:mt-0 sm:max-w-md xl:p-0 ">
                <div class="p-6 space-y-4 md:space-y-6 sm:p-8">
                    ...
                    <form class="space-y-4 md:space-y-6" action="#">
                        ...
                        <button type="submit" class="w-full text-white bg-blue-600 hover:bg-blue-700 focus:ring-4 focus:outline-none focus:ring-primary-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center" @click="onFormSubmit">{{$t("register.register_button")}}</button>
                        <p class="text-sm font-light text-gray-500 ">
                            {{ $t("register.already_have1") }} <router-link to="/login" class="font-medium text-primary-600 hover:underline "> {{ $t("register.already_have2") }}</router-link>
                        </p>    
                    </form>
                </div>
            </div>
